...
def get_neighbors(city):
    return list(graph[city].keys())

#====================================================
# =========== UNIVERSAL HILL CLIMBING ===============
#====================================================
def hill_climbing(start, goal):
    ...
    while True:
        ...
        if heuristic(best, goal) >= heuristic(current, goal):
            print("\nReached a local maximum before reaching the goal")
            return path, -1

        current = best
        path.append(current)

def measure_time(start, goal):
...
while True:
    ...
    if start_city in graph: break
    print("Invalid city.\n")

while True:
    goal_city = input("Enter GOAL city: ").strip()
    if goal_city in graph: break
    print("Invalid city.\n")

print("\nRunning Hill Climbing...\n")

path, cost, elapsed = measure_time(start_city, goal_city)

print("\n========== RESULT ==========")
print("Path:", " → ".join(path))
print("Cost (steps):", cost)
