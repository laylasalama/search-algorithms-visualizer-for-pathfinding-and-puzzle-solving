...
def run_ucs(grid, cost_grid, start, goal):
    """UCS wrapper compatible with grid_runner API.
    ...
    while frontier:
        ...
        for neighbor in get_neighbors(grid, current):
            step_cost = cost_grid[neighbor[0]][neighbor[1]]
            new_cost = current_cost + (step_cost if step_cost is not None else 1)
            if neighbor not in cost_so_far or new_cost < cost_so_far[neighbor]:
                cost_so_far[neighbor] = new_cost
                parent[neighbor] = current
                heappush(frontier, (new_cost, neighbor))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 5:
        ...
        run_ucs(grid, cost_grid, start, goal)
    else:
        # fallback to interactive mode
        grid, cost_grid, start, goal, algorithm = input_problem()

        if algorithm == 1:
            path, status = UCS_visual(grid, cost_grid, start, goal, delay=0.25, timeout=10)
            if status=="FOUND":
                print("\nUCS Found a Path!")
                print_grid_with_path(grid, start, goal, path)
                print(f"Path Length: {len(path)}")
            elif status=="TIMEOUT":
                print("\nUCS stopped: exceeded timeout.")
            else:
                print("\nUCS: No Path Found.")