...
#================================A* algorithm==========================
#======================================================================
#def A_st(maze, start, goal): return [], 0, 0
def A(maze, start, goal):      
    frontier = []
    # each node: (f_cost, g_cost, cell, parent)
    start_node = (heuristic_maze(start, goal), 0, start, None)
    ...
    while frontier:
        f_cost, g_cost, current, parent = heapq.heappop(frontier)
        ...
        for nxt in get_neighbors_maze(current, maze):
            tentative_g = g_cost + 1  # maze cost = 1 per step
            if nxt in g_costs and tentative_g >= g_costs[nxt]:
                continue
            g_costs[nxt] = tentative_g
            heapq.heappush(frontier, (tentative_g + heuristic_maze(nxt, goal), tentative_g, nxt, current))
